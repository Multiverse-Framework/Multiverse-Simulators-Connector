...
class GazeboCompiler(MultiverseSimulatorCompiler):
                ...
                if isinstance(body_data, dict):
                    ...
                    for attribute_name, attribute_value in body_data.items():
                        if attribute_name in ["pos", "quat"]:
                            continue
                        attribute_element = ET.SubElement(link_element, attribute_name)
                        attribute_element.text = (
                            attribute_value
                            if isinstance(attribute_value, str)
                            else (" ".join([*attribute_value]) if isinstance(attribute_value, (list, tuple)) else str(attribute_value))
                        )
    ...
    def apply_suffix_prefix(
        ...
        for link_element in entity_root.findall(".//link"):
            ...
            link_element.set("name", f"{prefix.get('body', '')}{link_name}{suffix.get('body', '')}")
        ...
        for visual_element in entity_root.findall(".//visual"):
            visual_name = visual_element.get("name", "")
            visual_element.set("name", f"{prefix.get('geom', '')}{visual_name}{suffix.get('geom', '')}")
        for collision_element in entity_root.findall(".//collision"):
            collision_name = collision_element.get("name", "")
            collision_element.set("name", f"{prefix.get('geom', '')}{collision_name}{suffix.get('geom', '')}")
        for joint_element in entity_root.findall(".//joint"):
    ...
    def add_entity(
        ...
        for entity_name, entity_data in entity.items():
            ...
            entity_file_ext = os.path.splitext(entity_file_name)[1]
            entity_file_path = os.path.join(self.save_dir_path, f"{entity_name}{entity_file_ext}")
            ...
            entity_model = entity_root.find("model")
            assert entity_model is not None, f"Model element not found in {entity_file_path}"
            entity_model.set("name", entity_name)
            for uri_element in entity_root.findall(".//uri"):
                ...
                if not os.path.isabs(uri_element.text):
                    entity_file_abspath = os.path.join(os.path.dirname(entity_data.path), uri_element.text)
